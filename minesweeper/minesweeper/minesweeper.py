...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        """
        if cell in self.safes and len(self.mines) == count:
            # 1)
            self.moves_made.add(cell)
            # 2)
            self.safes.add(self.mark_safe(cell))
            ...
            for sentence in self.knowledge:
                if count == 0:
                    self.mines.add(self.mark_mine(cell))
                else:
                    self.safes.add(self.mark_safe(cell))

            # 5)
            # iter knowledge, we need 2 for loop, if you Very confused,
            # Please open Python shell >>>
